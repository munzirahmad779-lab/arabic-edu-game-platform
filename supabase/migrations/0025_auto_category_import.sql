-- 0025_auto_category_import.sql
-- Ganti RPC import_question_bank_rows supaya:
--   1. Kolom Topik boleh kosong → soal tanpa kategori.
--   2. Kalau Topik berisi nama yang belum ada di question_categories
--      milik guru ini, otomatis dibuatkan kategori baru.
--   3. Kalau sudah ada, pakai yang existing.
...
begin
  ...
  for row_item in select value from jsonb_array_elements(p_rows) loop
    ...
    -- Topik: boleh kosong (soal tanpa kategori), atau maks 200 karakter.
  ...
  -- Auto-create kategori untuk guru ini dari setiap Topik unik yang tidak kosong.
  for new_topic in
